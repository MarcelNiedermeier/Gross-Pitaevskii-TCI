...
function evaluate_reduced_2D(qtt_xy, xygrid, R, prec, xmin, xmax, ymin, ymax)
    ...
    for i in 1:2^prec
        for j in 1:2^prec
            arr[j, i] = qtt_xy(QG.grididx_to_quantics(xygrid, (xindexgrid[i], yindexgrid[j])))
        end
    end
    return arr
end
...
compute_bond_dim = true

# choose at which points in time you would like to calculate the heatmaps!
heatmap_indices = [1] # which wave functions in the list of saved wave functions to evaluate
xreds = [30.]#, 5.]
yreds = [30.]#, 5.]
x0_red = 0.
y0_red = 0.
...
# load MPS wave functions -> specify path exactly 
cd("Runs/Data_Gaussian_moving_k5_quadratic_potential_2D_sine_potential_modulation_x_2D_sine_potential_modulation_y_2D_sine_potential_modulation_twist1_2D_sine_potential_modulation_twist2_2D_20_g_5_xy_100.0_Nsteps_20_maxdim_50_tol_1.0e-8_dt_0.01")
...
# evaluate position operator expectation value <x> and <y>
if compute_pos
...
# evaluate width in x and y-direction as sqrt(<x^2> - <x>^2) and sqrt(<y^2> - <y>^2)
if compute_width
...
# compute the full 2D heatmap of the wavefunction at the given heatmap indices (points in time)
...
# compute a reduced 2D heatmap/zoom of the wavefunction at the given heatmap indices (points in time)
...
# evaluate the bond dimension evolution
if compute_bond_dim
